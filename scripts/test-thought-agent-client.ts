...
assert(task.includes("`OK:preflight` and `CREDENTIAL_READY`"));
...
assert(task.includes("`OK:start` and `THOUGHT_INPUT_READY`"));
assert(task.includes("displayed verified input"));
assert(task.includes("the exact `THOUGHT_END` line"));
assert(task.includes("No CR/extra line/JSON/trim/repair/retry/replacement"));
assert(task.includes("Only `OK:result` and `Receipt:` succeed"));
...
assert(localTask.includes(Buffer.from(bootstrap.url, "utf8").toString("base64")));
