...
assert(task.includes("runtime metadataSource=reported"));
assert(task.includes("rawSha256/agentLineSha256 are sha256:<64 lowercase hex>"));
...
assert(task.includes("Retain its non-empty exact model as <runtime_model>"));
assert(task.includes("retain valid reasoning effort only if supplied"));
...
assert(task.includes("Use another chat turn only for an observed blocker"));
