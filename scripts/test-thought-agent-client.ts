...
assert(task.includes("Run control+one creative turn;"));
assert(task.includes("no readiness/CREATE confirmation"));
assert(!task.includes("Reply CREATE"));
assert(task.includes("No setup."));
assert(task.includes("exec_command(tty:true)=>live session_id"));
assert(task.includes("THOUGHT_CONTINUATION_READY"));
assert(task.includes("nonce via write_stdin"));
assert(task.includes("THOUGHT_CONTINUATION_OK+waits"));
assert(task.includes("write PROCEED; then claim"));
assert(task.includes("Same worker owns token/all ops/candidate"));
assert(task.includes("No proof=no claim"));
assert(task.includes("no heredoc"));
assert(task.includes("no raw/credential/file/log"));
assert(task.includes("Lost worker/private state=>terminal"));
assert(task.includes("intact=>Recovery"));
...
assert(task.includes("Serialize once"));
...
assert(task.includes("None => omit both"));
...
assert(task.includes("Absence valid"));
...
assert(task.includes("spec/instructions differ."));
