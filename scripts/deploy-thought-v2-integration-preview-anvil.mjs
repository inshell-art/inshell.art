...
async function main() {
  ...
  for (let index = 0; index < THOUGHT_PATH_FIXTURE_COUNT; index += 1) {
    const sparkName = `THOUGHT fixture ${index + 1}`;
    const expectedNameHash = keccak256(toUtf8Bytes(sparkName));
    await (await pathNft.allowSparker(fixtureOwner, sparkName)).wait();
    const tokenId = await fixturePathNft.mintSparker.staticCall(expectedNameHash, "0x");
    await (await fixturePathNft.mintSparker(expectedNameHash, "0x")).wait();
    const [owner, stage, stageMinted, sparker, storedSparkName] = await Promise.all([
      pathNft.ownerOf(tokenId),
      pathNft.getStage(tokenId),
      pathNft.getStageMinted(tokenId),
      pathNft.isSparker(tokenId),
      pathNft.sparkName(tokenId),
    ]);
    if (
  ...
  const runtime = {
    ...
    localLane: {
      ...
      pathRelease: {
        ...
        consumeAuthorizationSchema: pathRelease.pin.consumeAuthorizationSchema,
        pathNftRedeploymentRequired: pathRelease.pin.pathNftRedeploymentRequired,
      },
    },
    creativeSpec: {
