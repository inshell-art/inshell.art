 ...
 * readable here. Delivery uses a deterministic Brotli payload to stay within
 * the host's task-size limit; no code is downloaded or written to disk.
 */
...
export const THOUGHT_CODEX_TRANSPORT_WORKER_LOADER =
  'const{brotliDecompressSync}=require("node:zlib"),{createHash}=require("node:crypto"),{runInThisContext}=require("node:vm"),s=brotliDecompressSync(Buffer.from(process.argv[1],"base64"));if("sha256:"+createHash("sha256").update(s).digest("hex")!==process.argv[2])process.exit(70);process.argv.splice(1,2);runInThisContext(s,{filename:"thought-codex-transport-worker.js"})';
...
export function buildThoughtCodexTransportWorkerCommand(
  ...
  return [
    ...
    "--",
    shellQuote(THOUGHT_CODEX_TRANSPORT_WORKER_BROTLI_BASE64),
    shellQuote(THOUGHT_CODEX_TRANSPORT_WORKER_SHA256),
