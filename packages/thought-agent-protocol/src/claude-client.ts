...
const buildThoughtClaudeCoworkTask = (input: ThoughtClaudeTaskInput) => {
  ...
  return [
    ...
    "",
    "1. Claim control",
    "At <claim_endpoint>, submit one POST using <launch_credential> as Bearer authorization. Use exactly <claim_fields>, without shortening or renaming a field; fill them from <protocol> and the listed bridge/adapter values. Accept only runId=<run_id>, state=claimed, a non-empty top-level bridgeToken, and a bounded-preflight request using <control_schema>. The creative prompt must still be absent.",
