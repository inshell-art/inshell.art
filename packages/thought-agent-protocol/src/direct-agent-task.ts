...
export function buildThoughtDirectAgentTask(
  ...
  return [
    ...
    "",
    "4. Return once",
    "Return at <result_endpoint> with PUT, <bridge_credential>, and Idempotency-Key=<invocation_id>. Bind <protocol>, <invocation_id>, the exact claim bridge/adapter, <agent_product>/<agent_provider>, <runtime_model>, optional supplied effort, metadataSource=reported, the capsule execution policy, the exact startedAt, a current UTC completedAt, and the exact compact candidate. Supply lowercase sha256: hashes of both candidate bytes and agentLine bytes.",
