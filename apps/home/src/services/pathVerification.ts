...
export async function verifyPathRecord(
  ...
  const [observedChainIdValue, observedAtBlock] = await Promise.all([
    getChainId(provider),
    getBlockNumber(provider),
  ]);
  const [code, owner, tokenUri, transaction] =
    await Promise.all([
      getCode(provider, target.contract, observedAtBlock),
      readPathTokenOwner({
        provider,
        pathNftAddress: target.contract,
        tokenId: BigInt(target.tokenId),
        blockTag: observedAtBlock,
      }),
      readPathTokenUri({
        provider,
        pathNftAddress: target.contract,
