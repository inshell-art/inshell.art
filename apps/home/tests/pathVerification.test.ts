...
describe("PATH verification links", () => {
  ...
  test("verifies the pinned contract, token metadata, and matching mint transfer", async () => {
    ...
    expect(mockGetCode).toHaveBeenCalledWith(provider, CONTRACT, 99);
    expect(mockReadPathTokenOwner).toHaveBeenCalledWith(
      expect.objectContaining({ blockTag: 99 }),
    );
    expect(mockReadPathTokenUri).toHaveBeenCalledWith(
      expect.objectContaining({ blockTag: 99 }),
    );
  });
