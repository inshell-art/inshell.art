...
export const runThoughtConsoleTests = () => {
  ...
  const restoredWithoutLegacyLiveState = parseThoughtConsoleHistory(
    JSON.stringify({
      version: 1,
      entries: [
  ...
  assert.deepEqual(
    restoredWithoutLegacyLiveState,
    warningWithRecovery,
    "restoring history must remove legacy live projections and inferred boundaries",
  );
  ...
  assert.equal(restoredTerminalOutcome.entries.length, 1);
  assert.equal(
    restoredTerminalOutcome.entries[0]?.time,
    "20:03:00",
    "history restore keeps only the latest copy of an idempotent terminal outcome",
  );
  assert.equal(
    restoredTerminalOutcome.entries[0]?.nextStep,
  ...
  assert.equal(
    parseThoughtConsoleHistory(serializeThoughtConsoleHistory(warningWithRecovery), {
      limit: 2,
    }).entries.length,
    2,
