...
export const isThoughtLocalRuntimeHost = (hostname: string): boolean => {
...
const isLocalRuntimeRpc = (value: string) => {
  try {
    return isThoughtLocalRuntimeHost(new URL(value).hostname);
  } catch {
    return false;
  }
};
...
export const alignThoughtV2LocalRpcHost = (
  ...
  if (!isThoughtLocalRuntimeHost(browserHostname)) {
    return rpcUrl;
  }
  try {
    const parsed = new URL(rpcUrl);
    if (!isThoughtLocalRuntimeHost(parsed.hostname)) {
      return rpcUrl;
    }
...
export const isThoughtV2LocalMintRuntime = (
  ...
  return facts.dev &&
    isThoughtLocalRuntimeHost(facts.hostname) &&
